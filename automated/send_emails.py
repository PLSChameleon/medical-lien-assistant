...
# Robust, flushed logging for successful sends
def log_success(pid, to_email, subject):
    ...
    try:
        with open("sent_emails.log", "a", encoding="utf-8") as log:
            ...
            log.flush()
            os.fsync(log.fileno())
        print(f"📝 Logged to sent_emails.log: PID {pid}")
    except Exception as e:
        print(f"❌ Failed to write to sent_emails.log for PID {pid}: {e}")

# Load sent PIDs from log
sent_pids = set()
logged_pids = set()  # track PIDs actually in file
...
# ✅ Final pass: re-log any PIDs sent but not in file
